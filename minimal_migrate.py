...
######################
# Small changes made by me vs the original to use the ORM generated tables
...
def dumb_migrate_db(allow_deletions=False):
    ...
    migration.
    """
    # make it work with SQLAlchemy 2.0
    with sqlite3.connect(buddy.db.LOCAL_SQLITE_DB) as real_db:
        with DBMigrator(real_db, None, allow_deletions) as migrator:
            migrator.migrate()
